function MenuState(name) {
	...
	var _yPos = 100;
	btns.push(new MenuButton("Un singur jucator", 20, _yPos, function() {
		state.get("game").init(ONE_PLAYER);
		state.change("game");
	}));
	btns.push(new MenuButton("Doi jucatori", 20, _yPos+70, function() {
		state.get("game").init(TWO_PLAYER);
		state.change("game");
	}));
	btns.push(new MenuButton("Despre", 20, _yPos+140, function() {
		state.change("about", true);
	}));
	btns.push(new MenuButton("Creat de Mihai Cornel", 20, _yPos+210));
...
function GameState(name) {
	...
	this.update = function() {
		...
		if (!activeAnim) {
			...
			if (winner && !aiMoved) {
				if (winner === true) {
					winnerMsg = "Este EGALITATE!";
				} else if (winner === Tile.NOUGHT) {
					winnerMsg = "Jucatorul cu 0 a invins!";
				} else {
					winnerMsg = "Jucatorul cu X a invins!";
				}
			}
